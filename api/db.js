...
module.exports = async (req, res) => {
  ...
  // instead of its own api/test-jsonbin.js (Vercel Hobby caps this project at
  // 12 functions; a 13th fails the whole deployment). Deliberately placed
  // BEFORE the x-db-key gate so it can be checked straight from a browser.
  // That is safe only because, unlike the old standalone file (which echoed
  // the first 100 bytes of the raw bin), this NEVER returns record contents —
